...
多页签切走真的保状态，原生跑 SQL Server。**357 条自动化测试全打真实依赖，不 mock。**
...
and **357 automated tests that run against real dependencies, with no mocks**.
...
**Tests:** 303 pytest cases (real SQL Server) + 54 Playwright cases (isolated second
...
| **测试** | 模板通常不带测试；带的多到组件单测为止（jsdom + mock fetch） | **357 条打真实依赖、零 mock** —— pytest 303 条对真实 SQL Server，Playwright 54 条对真实浏览器 + 真实接口 + 真实库。数据权限是拿 **19 个真账号**跑出来的；授权层（RBAC 四道闸、权限码三方对账、生产配置校验）另有 68 条安全测试兜底 |
...
**357 条自动化测试**，两边都不 mock 依赖 —— 后端对真实 SQL Server，
前端对真实浏览器 + 真实接口 + 真实数据库。
