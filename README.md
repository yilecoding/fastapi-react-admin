...
多页签切走真的保状态，原生跑 SQL Server。**325 条自动化测试全打真实依赖，不 mock。**
...
[![TypeScript](https://img.shields.io/badge/TypeScript-6-3258C6?style=flat-square&logo=typescript&logoColor=white)](https://www.typescriptlang.org)
...
and **325 automated tests that run against real dependencies, with no mocks**.
...
**Tests:** 271 pytest cases (real SQL Server) + 54 Playwright cases (isolated second
...
| **测试** | 模板通常不带测试；带的多到组件单测为止（jsdom + mock fetch） | **325 条打真实依赖、零 mock** —— pytest 271 条对真实 SQL Server，Playwright 54 条对真实浏览器 + 真实接口 + 真实库。数据权限是拿 **19 个真账号**跑出来的；授权层（RBAC 四道闸、权限码三方对账、生产配置校验）另有 68 条安全测试兜底 |
...
**325 条自动化测试**，两边都不 mock 依赖 —— 后端对真实 SQL Server，
前端对真实浏览器 + 真实接口 + 真实数据库。

| | 跑在哪 | 条数 | 覆盖 |
|---|---|---|---|
| **pytest** | 真实 SQL Server（`fba_test` 库，不碰开发库） | **271** | 安全（RBAC 门禁 / 权限码对账 / 数据权限 fail-closed / 规则校验 / 生产配置 / 健康检查 / 缓存失效）75 · 定时任务 74 · 文件模块 30 · 数据权限端到端 29 · 分页扇出 4 · 认证 11 · 个人中心信封契约 5 · 高危写接口 9 · 动态配置容灾 3 · 消息通知 9 · 迁移守卫 7 · 种子方言一致性 4 · i18n 对称性 2 · 线上格式（ID 字符串 / 时区标记）2 · 其他 7 |
...
```bash
pnpm test          # 后端 pytest（271 条，需要 fba_test 库）
