...
async def create_data_rule(
    db: CurrentSessionTransaction, obj: CreateDataRuleParam
) -> ResponseSchemaModel[GetDataRuleDetail]:
    # 返回新建的规则 —— 「新建规则并挂到当前数据范围」需要立刻拿到它的 id
    rule = await data_rule_service.create(db=db, obj=obj)
