...
class OperaLogService:
    ...
    async def get_list(
        *,
        ...
        :param status: 状态
        :param ip: IP 地址
        :param start_time: 操作时间起
        :param end_time: 操作时间止
        :return:
        """
        log_select = await opera_log_dao.get_select(
            username=username, status=status, ip=ip, start_time=start_time, end_time=end_time
        )
