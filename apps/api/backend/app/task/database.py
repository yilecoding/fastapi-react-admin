...
class DatabaseBackend(BaseBackend):
    """重写 celery.backends.database 的 DatabaseBackend。

    沿用上游的实现（理由同上游：celery 自带那个「与模型配合不佳，导致创建表和
    alembic 迁移困难」）。模型换成了 `model/result.py` 里改过 SQL Server 账的那份，
    建表交给 `MappedBase.metadata.create_all()`，所以 `SessionManager.prepared`
    被置成 True，不让 celery 自己建。
    """
