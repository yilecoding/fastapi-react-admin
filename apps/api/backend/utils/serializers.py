...
def stringify_unsafe_ints(obj: Any) -> Any:
    """
    把超出 JS 安全整数范围的 int 转成 str，递归处理。

    雪花 ID 约 2^61，远超 JS 的 Number.MAX_SAFE_INTEGER (2^53-1)。
    浏览器 JSON.parse 会**静默丢精度**，实测：

        2049629108245233664 → 2049629108245233700
        2049629108245233665 → 2049629108245233700   ← 不同 ID 塌缩成同一个值

    后果不只是列表渲染撞 key：把 ID 回传做更新/删除会命中错误记录。

    FBA 自带的 `common/schema.py: field_serializer('id')` 只覆盖 Pydantic 模型的
    `id` 字段，**外键（dept_id / parent_id / role_id…）和裸 dict 接口都漏了**。
    在编码层统一处理才能兜住全部出口。

    注意 bool 是 int 的子类，必须排除。
    """
    ...
    if isinstance(obj, dict):
        return {k: stringify_unsafe_ints(v) for k, v in obj.items()}
    if isinstance(obj, (list, tuple)):
        return [stringify_unsafe_ints(v) for v in obj]
    return obj
...
class MsgSpecJSONResponse(JSONResponse):
    ...
    渲染前会把超范围整数转成字符串，见 `stringify_unsafe_ints`。
    """

    def render(self, content: Any) -> bytes:
        return json.encode(stringify_unsafe_ints(content))
