...
class OperaLogMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Any) -> Response:  # ruff:ignore[complex-structure]
        ...
        elapsed = 0
        request_headers = self.collect_headers(request.headers)
        ...
        try:
            ...
            if should_log_opera and method != 'OPTIONS':
                response_headers = self.collect_headers(response.headers)
                response, response_body = await self.capture_response_body(response)
    ...
    def collect_headers(headers: Any) -> dict[str, str]:
        ...
        `OPERA_LOG_REDACT_HEADERS` 里的头会被整体打码 —— 它们要么是凭据
        （authorization / x-api-key），要么含会话信息（cookie）。
        直接落库会让操作日志本身变成凭据泄露面。
        """
        redact = {h.lower() for h in settings.OPERA_LOG_REDACT_HEADERS}
        out: dict[str, str] = {}
    ...
    @staticmethod
    async def capture_response_body(response: Response) -> tuple[Response, str | None]:
        """
        捕获响应体，并把响应重新组装回去。

        `call_next` 返回的是流式响应，`body_iterator` 只能消费一次 ——
        消费之后必须用同样的内容重建 Response，否则客户端会收到空响应。

        两条保护：
        1. 只记 JSON / 纯文本类响应 —— 文件下载、SSE 之类**完全不碰**，
           连迭代器都不消费，直接原样放行（否则会把大文件读进内存，或打断流式推送）
        2. 超过 `OPERA_LOG_RESPONSE_MAX_SIZE` 的部分截断，只留提示
        """
        content_type = (response.headers.get('content-type') or '').split(';')[0].strip().lower()
        if content_type not in settings.OPERA_LOG_RESPONSE_CONTENT_TYPES:
    ...
    def truncate_body(raw: bytes) -> str | None:
        ...
        if len(raw) > limit:
            ...
            return f'{head}\n\n…[已截断，完整响应 {len(raw)} 字节，上限 {limit} 字节]'
