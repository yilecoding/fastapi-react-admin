...
### 硬纪律 6（ID 一律下发字符串）现在有专门的守卫

`backend/tests/test_snowflake_wire_format.py`：从 OpenAPI **枚举**所有无参 GET
端点（44 个），逐个请求、递归走 JSON，断言不存在超出 `2^53-1` 的裸整数。
新加只读接口自动罩上，不用改清单。

为什么补：实测把 `stringify_unsafe_ints` 改成 `return obj`（大整数原样下发），
全套 261 条里只有 `test_file.py` 的 **2 条**红 —— 而它们红是因为恰好
`assert isinstance(data['id'], str)`，不是在测这件事。那两条断言一改，
全仓引用最多的那条硬纪律就没人看着了。

它同时示范了「先断言『有』」：除了「没有裸大整数」，还断言
**扫到的端点数 ≥ 30** 且**见到的雪花字符串 ≥ 20** —— 否则库空了、
