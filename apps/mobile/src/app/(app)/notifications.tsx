...
export default function NotificationsScreen() {
  ...
  return (
    <View className="bg-panel flex-1">
      <View className="border-line flex-row items-center gap-2 border-b px-5 py-3">
      ...
      >
        {/* 硬纪律 9：失败必须是**可见状态**，不是缺失状态 —— 不能让「拉取失败」
            和「一条通知都没有」长得一样 */}
        {error ? (
          <View className="border-destructive/40 bg-destructive/10 mt-4 gap-2 rounded-xl border p-3">
          ...
          <View className="py-20">
            <Text className="text-faint text-center text-sm">
          ...
          items.map((n, i) => <Row key={n.id} n={n} first={i === 0} onPress={() => void markRead(n)} />)
        )}
      </ScrollView>
    </View>
  )
...
/**
 * 一条通知。**不是卡片** —— 左边一条导轨，每条用一枚刻度钉在上面，
 * 未读那枚是主色实心。这是 web 面板里那条 rail 在列表上的形态。
 */
function Row({ n, first, onPress }: { n: Notification; first?: boolean; onPress: () => void }) {
  const unread = !n.read_time
  return (
    <Pressable onPress={onPress} className="active:bg-node flex-row gap-3.5">
      {/* 导轨：一条贯穿整列的发丝线，刻度落在标题那一行的高度上 */}
      <View className="w-2 items-center">
        <View className={`bg-line w-px flex-1 ${first ? 'mt-5' : ''}`} />
        <View
          className={`absolute top-4 h-2 w-2 rounded-full ${unread ? 'bg-accent' : 'bg-line'}`}
        />
        <View className="bg-line w-px flex-1" />
      </View>
      <View className="border-line flex-1 gap-1.5 border-b py-3.5">
        ...
        <Text className={`text-ink text-sm ${unread ? 'font-semibold' : ''}`}>{n.title}</Text>
        <Text className="text-dim text-sm leading-5" numberOfLines={2}>
          {n.content}
        </Text>
      </View>
    </Pressable>
