...
export default function EditProfileScreen() {
  ...
  async function save() {
    ...
    try {
      ...
      if (avatarChanged) {
        // 🔴 空串要发 `null`，**不能发 `''`**。
        // 读取侧 `GetUserInfoDetail.avatar` 是 `HttpUrl | None`，存进一个空串之后
  ...
  return (
    ...
    >
      <ScrollView contentContainerClassName="gap-5 p-4" keyboardShouldPersistTaps="handled">
        ...
        <Field
          label="头像地址"
          hint="留空表示清除头像。暂时只能填 URL —— 从相册选图要 expo-image-picker + 文件上传接口，还没接。"
        >
          <Input
            value={avatar}
        ...
        <Button disabled={!dirty || saving} onPress={() => void save()} testID="edit-save">
