 ...
 *
 * `Modal` 会把内容挂到**另一个原生根视图**上，而那一类「样式在另一个原生根上失效」
 * 的问题**打包和 typecheck 全都不报**。所以选了已经在用的那条路：
 * `@rn-primitives/portal` 渲染进根 layout 的 `<PortalHost />`，
 * **同一棵 React 树、同一个原生根**，令牌解析和普通屏一模一样。
 * 代价是 Android 返回键要自己接（见下）。
 *
 * ✅ **这条已经在 Android 设备上验收过了**（层级正确、返回键关的是弹框而不是退屏）。
 * 当初它是「无法验证前提下的选择」，现在是结论 —— 但**只在 Android 上**，
 * iOS 一次没跑过（见 `scripts/` 分册的设备一节）。
 *
 * ## 🔴 Android 的返回键必须接
 *
