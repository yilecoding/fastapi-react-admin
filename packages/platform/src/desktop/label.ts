 ...
 * 判据和 `_shared` 那个被删掉的只读列表工厂不同：
 *
 * | | 那个工厂 | 这里 |
 * |---|---|---|
 * | 里面有什么 | 一层可以随时重写的组装逻辑 | 上面那条 **203dpi 重采样**的结论 —— 删了就得再踩一次 |
 * | 为什么没人用 | 真页面长出了自己那版，它被绕过了 | 业务页面还没有，而**静默打印正是 `apps/desktop` 存在的理由** |
 *
 * 真到接第一个打印页时，从这里开始改而不是重写。
 */
