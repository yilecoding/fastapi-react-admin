...
**症状**：侧栏写了 `sticky top-2 self-start`，但滚动时它照样跟着滚走 ——
看着像「sticky 这个类没生效」，于是去查有没有祖先 `overflow:hidden`
（那是 sticky 最有名的那个坑），查半天什么都没有。

**根因**：**元素本身比视口高**。sticky 是「滚到 `top` 位置就停住」，
而一个 880px 高的侧栏在 840px 的可视区里**没有可粘的余量** ——
它得先整个滚过去，才谈得上停。CSS 层面完全合法，所以没有任何报错。

组件沙箱的左栏踩到过：49 个条目铺开约 1900px，
于是「想看复杂组件得把整页往下拖，拖的时候右边的舞台和代码一起滚走了」。

**修法**（和 `_shared/master-list.tsx` 的左栏同一套骨架）：

```
外层  sticky top-2 self-start + max-h-[calc(100svh-7.5rem)]   ← 高度上限是前提
 ├ 搜索/标题  shrink-0                                          钉住
 └ 列表      min-h-0 + flex-1 + overflow-y-auto                只有这里滚
```

三处都不能省：`max-h` 让 sticky 有余量，`min-h-0` 让列向 flex 的项肯收缩
（少了它 `overflow-y-auto` 永远不触发），`overflow-x-hidden` 防那条白得的横向滚动条。

⚠️ 顺带两条：

- **`max-h` 要按断点分叉。** 窄屏是堆叠布局，限死 `100svh` 会让侧栏内滚一屏、
  把正文顶到很下面；给个 `max-h-[60svh]`，`lg:` 起再换成减掉外壳
  （顶栏 3rem + 标签条约 2.5rem）的那个值
- **列表能滚之后要把「当前项」滚进视野**，否则直接打开
  `?c=tree` 停在最顶上，「我在哪」看不出来。用 ref + 手算
  `scrollTop`，**不要 `el.scrollIntoView()`** —— 它会连外层能滚的祖先一起滚，
  右边的内容跟着跳一下；也**不要 `document.querySelector`**（硬纪律 5）
