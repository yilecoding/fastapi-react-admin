...
import { useIsMobile } from "@admin/ui/hooks/use-mobile"
...
function SidebarInset({ className, ...props }: React.ComponentProps<"main">) {
  return (
    <main
      ...
      className={cn(
        // min-w-0 必须有：inset 是 wrapper 这一行 flex 里的项，
        // `min-width: auto` 会让它拒绝收缩到内容宽度以下 —— 任何一处宽内容
        // （标签条上十几个 tab、12 列的宽表）都会把它顶过视口，
        // 侧边栏被挤、右侧内容跑到屏幕外。内部该滚的地方各自有 overflow。
